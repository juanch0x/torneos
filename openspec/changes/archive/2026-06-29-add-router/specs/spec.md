# Router Specification — `add-router`

## Purpose

Requirements for introducing TanStack Router as the navigation layer.
URL becomes the navigation signal; `current` in the store becomes a data cache.
The current ternary (`current ? <TournamentView /> : <TournamentList />`) is replaced by route-driven rendering.
